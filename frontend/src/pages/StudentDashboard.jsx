...
import { FileText, Upload, ChevronRight, X, Edit2, Loader2, AlertCircle, Sparkles, GitBranch, Target, Brain, Activity } from 'lucide-react';
                      ...
                      {question.options?.map((opt, i) => (
                        <label key={i} className="flex items-start gap-2 cursor-pointer p-2 rounded bg-white/5 hover:bg-white/10 transition-colors">
                          <input type="radio" name={question.id} value={opt} className="mt-0.5"
                            ...
                            onChange={() => setAssessmentAnswers(prev => ({ ...prev, [question.id]: { answer: opt, selectedIndex: i } }))}
                          />
